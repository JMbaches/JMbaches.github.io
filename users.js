  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px">
    ${users.map(u=>{
      ...
      return `<div class="card" style="padding:14px">
        ...
        ${u.posteAtelier?`<div style="margin-bottom:4px"><span style="font-size:10px;padding:1px 6px;border-radius:3px;background:var(--accent-light);color:var(--accent-deep);font-weight:600">Poste atelier : ${{tablier:'Tablier',accessoires:'Accessoires',axes:'Axes'}[u.posteAtelier]||u.posteAtelier}</span></div>`:''}
...
async function saveUser() {
  ...
  if(editingUserId) {
    ...
    u.name=name; u.login=login; u.email=email; u.role=role; u.perms=perms; u.perimetre=perimetre; u.posteAtelier=posteAtelier;
    ...
    try {
      ...
      const profileData = { name, login, email, role, perms, perimetre, posteAtelier, active: true };
